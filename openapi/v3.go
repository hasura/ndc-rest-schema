...
func (oc *openAPIv3Converter) convertProcedureOperation(pathKey string, method string, operation *v3.Operation) (*types.RESTProcedureInfo, error) {
	...
	if reqBody != nil {
		...
		if paramData, ok := arguments["data"]; ok {
			arguments["paramData"] = paramData
		}

		arguments["data"] = schema.ArgumentInfo{
...
func (oc *openAPIv3Converter) getSchemaTypeFromProxy(schemaProxy *base.SchemaProxy, nullable bool, fieldPaths []string) (schema.TypeEncoder, *base.Schema, error) {
	...
	if refName != "" && len(innerSchema.Type) > 0 && innerSchema.Type[0] == "object" {
		...
		if innerSchema.Title != "" && !strings.Contains(innerSchema.Title, " ") {
			fieldPaths = []string{toUnderscoreRegexp.ReplaceAllString(innerSchema.Title, "_")}
		}
